...
class StartButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(105, 105, 105),(105, 105, 105)], True)
...
class GiveupButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(105, 105, 105),(105, 105, 105)], False)
...
class Game():
    def __init__(self, caption):
        ...
        self.buttons = []
        self.buttons.append(StartButton(self.screen, 'Begin', MAP_WIDTH + 30, 100))
        self.buttons.append(GiveupButton(self.screen, 'End', MAP_WIDTH + 30, BUTTON_HEIGHT + 180))
        self.is_play = False

        self.map = Map(CHESS_LEN, CHESS_LEN)
        self.player = MAP_ENTRY_TYPE.MAP_PLAYER_ONE
        self.action = None
        self.AI1 = AI1(CHESS_LEN)
        self.AI2 = AI2(CHESS_LEN)
        self.useAI = True #change
        self.winner = None
    ...
    def play(self):
        ...
        if self.is_play and not self.isOver():
            if self.player == MAP_ENTRY_TYPE.MAP_PLAYER_ONE:
                x, y = self.AI1.findBestChess(self.map.map, self.player)
                self.checkClick(x, y, True)
            if self.player == MAP_ENTRY_TYPE.MAP_PLAYER_TWO:
                x, y = self.AI2.findBestChess(self.map.map, self.player)
                self.checkClick(x, y, True)
    ...
    def checkClick(self,x, y, isAI=True):
        self.map.click(x, y, self.player)
        if self.AI1.isWin(self.map.map, self.player):
    ...
    def showWinner(self):
        def showFont(screen, text, location_x, locaiton_y, height):
            ...
            font_image = font.render(text, True, (0, 0, 0), (255, 255, 255))
        ...
        if self.winner == MAP_ENTRY_TYPE.MAP_PLAYER_ONE:
            ...
            str = 'Winner is Black'
        showFont(self.screen, str, MAP_WIDTH + 25, SCREEN_HEIGHT - 160, 30)
        pygame.mouse.set_visible(True)
